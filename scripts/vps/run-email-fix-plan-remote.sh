#!/usr/bin/env bash
# С Mac: Email Fix Plan на VPS (sync SMTP + recreate + verify).
# Папка:
#   cd /Users/vladimirchudinov/Desktop/my-startup/my-app
#   bash scripts/vps/run-email-fix-plan-remote.sh
# С паролем:
#   VPS_SSH_PASSWORD='***' bash scripts/vps/run-email-fix-plan-remote.sh
# Без git pull (inline body):
#   VPS_SSH_PASSWORD='***' bash scripts/vps/run-email-fix-plan-remote.sh --inline
set -euo pipefail
...
if [[ "$INLINE" -eq 1 ]]; then
  BODY="${ROOT}/scripts/vps/remote-email-fix-plan-body.sh"
else
  REMOTE="cd /root/zeip/my-app && git pull --ff-only && bash scripts/vps/run-email-fix-plan-on-vps.sh"
fi
