#!/usr/bin/env bash
# Inline body для run-email-fix-plan-remote.sh --inline.
# Весь код выполняется на VPS через «ssh bash -s» — файлы на сервере не нужны.
# Не запускать напрямую на VPS; используйте run-email-fix-plan-remote.sh --inline с Mac.
set -euo pipefail
...
if grep -q 'p=quarantine' <<< "${DMARC:-}"; then
  echo "WARN: DMARC p=quarantine — откатите в Timeweb на p=none вручную"
...
# --- sync SMTP (same as remote-sync-smtp-body.sh) ---
...
HELO="$(grep "^SMTP_HELO_NAME=" "$APP_ENV" 2>/dev/null | head -1 | cut -d= -f2- || true)"
...
echo "=== verify transactional email ==="
sleep 3
...
echo "=== Email Fix Plan inline: DONE ==="
