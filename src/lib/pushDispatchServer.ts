...
/** Отправка Web Push всем подписчикам чата (кроме отправителя). */
export async function dispatchPushForMessage(
  messageId: string,
): Promise<PushDispatchResult> {
  const trimmed = messageId.trim();
  if (!trimmed) {
    return { ok: false, error: "message_id required", status: 400 };
  }
  ...
  const title = fullName !== "" ? fullName : "Новое сообщение в zeip";
  const bodyText =
    roleTitle !== "" ? `${roleTitle} · Новое сообщение` : "Новое сообщение";
