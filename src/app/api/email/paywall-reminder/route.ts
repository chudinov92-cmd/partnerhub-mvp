...
export async function POST(req: Request) {
  try {
    ...
    for (const row of profiles ?? []) {
      ...
      if (!result.ok) {
        console.error("[email/paywall-reminder] send failed", row.id, result.error);
        skipped += 1;
        continue;
      }
      ...
      if (updateErr) {
        console.error("[email/paywall-reminder] update flag", row.id, updateErr);
        skipped += 1;
        continue;
      }
    ...
    return jsonRouteError("[email/paywall-reminder]", e);
  }
}
